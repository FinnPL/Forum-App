...
class FullScreenPostWidget extends StatefulWidget {
  final Post? post;
  final String? id;
  const FullScreenPostWidget({Key? key, this.post, this.id}) : super(key: key);
...
class FullScreenPostWidgetState extends State<FullScreenPostWidget> {
  ...
  void initState() {
    ...
    if (widget.post != null) {
      isLoaded = true;
      post = widget.post!;
      date = LocalServices().getFormattedDate(post.date);
      getData();
    } else {
      RemoteService().getPost(widget.id!).then((value) {
        setState(() {
          post = value;
          date = LocalServices().getFormattedDate(post.date);
          isLoaded = true;
                              ...
                              child: GestureDetector(
                                onTap: () async {
                                  Navigator.push(
                                    ...
                                    MaterialPageRoute(
                                      builder: (context) =>
                                          FullScreenImage(image: imageBytes!),
                                    ),
                                  );
                                },
                                child: Hero(
